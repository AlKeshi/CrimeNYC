...
print("Loading NYC crime data...")
train_X_crimes_only = np.load('../data/train_data.npy')
test_X_crimes_only = np.load('../data/test_data.npy')

# Get dimensions from data
n_y_cells, n_x_cells = train_X_crimes_only.shape[1], train_X_crimes_only.shape[2]
print(f"Data dimensions: {n_y_cells} x {n_x_cells}")
print(f"Training samples: {len(train_X_crimes_only)}")
print(f"Testing samples: {len(test_X_crimes_only)}")
...
print(f"Mask created: {np.sum(mask)} cells inside NYC")
